        ...
        try:
            if dataframe is not None and not dataframe.empty:
                ...
                for r_index, c_index in enumerate(dataframe):
                    ...
                    max_len = max(series.apply(self._get_data_gbk_len).max(), self._get_data_gbk_len(series.name)) + 5
                    fm = None
                    if is_number_style:
                        # 数字增加千位符
                ...
                self._sheet_index += 1
                print('info: write data to sheet with style, successfully')
                return True
            else:
                print(f'info: dataframe is null _ {sheet_name}')
        except Exception as error:
            print(error)
            print(f'warning: some error on writing data to {sheet_name}')
...
if __name__ == '__main__':
    ...
    with Workbook('test') as w:
        w.write_to_sheet_with_style(df, 'test_data', is_number_style=True)
